...
def sample_discrete_features(probX, probE, node_mask):
    ...
    # zwh change
    # 加了条件之后，y的维度有所变化。0 --> num_classes
    return PlaceHolder(X=X_t, E=E_t, y=torch.zeros(bs, 6).type_as(X_t))
...
def gaussian_posterior_geom(X0, Xt, alpha_s_bar, alpha_t_bar, beta_t, node_mask):
    """
    Compute continuous posterior for geometry dims (DDPM closed form).
    X0: (bs,n,10)   - original clean geometry
    Xt: (bs,n,10)   - noisy z_t geometry
    alpha_s_bar: (bs,1,1)
    alpha_t_bar: (bs,1,1)
    beta_t:       (bs,1,1)
    """
    # coefficients
    coeff1 = (torch.sqrt(alpha_s_bar) * beta_t) / (1 - alpha_t_bar)
    coeff2 = (torch.sqrt(1 - beta_t) * (1 - alpha_s_bar)) / (1 - alpha_t_bar)

    # posterior mean μ_{t-1}
    mu = coeff1.unsqueeze(-1) * X0 + coeff2.unsqueeze(-1) * Xt

    # posterior variance σ^2
    var = beta_t * (1 - alpha_s_bar) / (1 - alpha_t_bar)
    var = torch.clamp(var, min=1e-8).unsqueeze(-1)

    # sample noise (optional, for training just return mean)
    eps = torch.randn_like(X0) * node_mask.unsqueeze(-1)

    X_s = mu + torch.sqrt(var) * eps
    return X_s, mu, var


def posterior_distributions(X, E, y, X_t, E_t, y_t, Qt, Qsb, Qtb, noisy_data):
    bs, n, dx = X.shape
    device = X.device
    # ============================
    # 1. split X
    ...
    # 3. 离散 posterior (类型)
    # ============================
    prob_X_type = compute_posterior_distribution(M=X_type, M_t=Xt_type, Qt_M=Qt.X, Qsb_M=Qsb.X, Qtb_M=Qtb.X)   # (bs, n, dx)
    
    # ============================
    # 4. 连续 posterior (几何)
    # ============================
    X_s_geom, mu_geom, var_geom = gaussian_posterior_geom(
        X0=X_geom,
        Xt=Xt_geom,
        alpha_s_bar=noisy_data['alpha_s_bar'],
        alpha_t_bar=noisy_data['alpha_t_bar'],
        beta_t=noisy_data['beta_t'],
        node_mask=y.new_ones(bs,n)
    )

    # ============================
    # 5. 组合
    # ============================
    prob_X = torch.zeros_like(X)
    prob_X[..., :3] = prob_X_type
    prob_X[..., 3:] = mu_geom  # 或 X_s_geom （采样用 X_s_geom）    
