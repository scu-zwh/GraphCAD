...
class LiftedDenoisingDiffusion(pl.LightningModule):
    ...
    def training_step(self, data, i):
        ...
        normalized_data = utils.normalize(X, E, data.y, self.norm_values, self.norm_biases, node_mask, self.dataset_info)
    ...
    def reconstruction_logp(self, data, data_0, gamma_0, eps, pred_0, node_mask, epsilon=1e-10, test=False):
        ...
        unnormalized_data = utils.unnormalize(X, E, y, self.norm_values, self.norm_biases, node_mask, collapse=False, dataset_info=self.dataset_info)
        unnormalized_0 = utils.unnormalize(X_0, E_0, y_0, self.norm_values, self.norm_biases, node_mask, collapse=False, dataset_info=self.dataset_info)
    ...
    def sample_batch(self, batch_id: int, batch_size: int, keep_chain: int, save_final: int, number_chain_steps: int,
        ...
        assert (E == torch.transpose(E, 1, 2)).all()  # 对称
        ...
        # for i in range(min(1, X.shape[0])):
        #     print("E: ", E[i])
        #     print("X: ", X[i])
    ...
    def sample_discrete_graph_given_z0(self, X_0, E_0, y_0, node_mask):
        ...
        """
        zeros = torch.zeros(size=(X_0.size(0), 1), device=X_0.device)  # 时间0
        ...
        sampled = utils.unnormalize(sampled.X, sampled.E, sampled.y, self.norm_values,
                                    self.norm_biases, node_mask, collapse=False, dataset_info=self.dataset_info)
        return sampled
