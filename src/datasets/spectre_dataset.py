...
class SpectreGraphDataset(InMemoryDataset):
    ...
    def download(self):
        ...
        # 本地保存原始数据的目录
        import shutil
        local_url_dir = "/mnt/data/zhengwenhao/workspace/DiGress/url_file"  # 你自己的本地路径
        ...
        if self.dataset_name == 'sbm':
            ...
            filename = 'community_12_21_100.pt'
        else:
            raise ValueError(f'Unknown dataset {self.dataset_name}')

        src_path = os.path.join(local_url_dir, filename)
        dst_path = os.path.join(self.raw_dir, filename)
        ...
        file_path = dst_path  # 与 download_url 保持一致
