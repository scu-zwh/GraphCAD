...
class CADGraphDataset(InMemoryDataset):
    ...
    def __init__(self, split, root, file_list_path, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.train_data = all_metrics_data['train']
        self.validation_data = all_metrics_data.get('validation', {})
        self.test_data = all_metrics_data.get('test', {})
        
        self.normalization_param = {'area': [2.1076135382373433, 1.8197015965982655],
                            'vol': [0.15210068573918173, 0.23091717061075676]} 
    ...
    def process(self):
        ...
        for i, rel_path in enumerate(tqdm(graph_rel_paths, desc=f"Loading {self.split} graphs")):
            ...
            cad_vec = torch.tensor(cad_vec, dtype=torch.int32).unsqueeze(0)  # shape (1, seq_len)
            
            # condition
            metrics_data = self.train_data.get(rel_path) or self.validation_data.get(rel_path) or self.test_data.get(rel_path)
            sv_data = {'area': metrics_data["area"], 'vol': metrics_data["vol"]}
            sv_data = self.normalize_data(sv_data)
            
            conditions = torch.tensor([sv_data["area"], sv_data["vol"],
                        metrics_data["planar_faces"], metrics_data["curved_faces"],
                        metrics_data["linear_edges"], metrics_data["curved_edges"],], dtype=torch.float32)
            ...
            try:
                ...
                if getattr(data, "y", None) is None:
                    data.y = conditions.unsqueeze(0)  # shape (1, num_conditions)
                    
                if getattr(data, "cad_vec", None) is None:
                    data.cad_vec = cad_vec  # shape (1, seq_len)
